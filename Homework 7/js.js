...
const map = {
    ...
    render(snakePointsArray, foodPoint, obstaclesPoint) {
        ...
        const obstaclesCell = this.cells[`x${obstaclesPoint.x}_y${obstaclesPoint.y}`];
...
const game = {
    ...
    reset() {
        ...
        this.obstacles.setCoordinates(this.getObstacles());
        this.render();
    },
    ...
    getObstacles() {
        const exclude = [this.obstacles.getCoordinates(), ...this.snake.getBody()];
        ...
        while (true) {
            ...
            if (!exclude.some(exPoint => obstPoint.x === exPoint.x && obstPoint.y === exPoint.y)) {
                return obstPoint;
            }
        }
    },
    ...
    tickHandler() {
        ...
        var wonPointsInfo = document.getElementById('wonPointsInfo');
        wonPointsInfo.innerHTML = (this.snake.getBody().length) - 1;
    ...
    canMakeStep() {
        ...
        return !this.snake.isOnPoint(nextHeadPoint) &&
            ...
            nextHeadPoint.y >= 0
            &&
            !(nextHeadPoint.x == this.obstacles.x &&
            nextHeadPoint.y == this.obstacles.y);
    },
